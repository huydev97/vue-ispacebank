...
export default {
   ...
   */
    ...
    /*
        lấy dữ liệu interest
        options: {
            idUser: Number
        }
    */
    async getInterestList({ commit, state }, options) {
        let results = await axios({
            method: 'POST',
            url: `${config.host}/bagbitcoin/getHistoryMyInterests`,
            data: {
                login_token: VueCoookie.get('login_token'),
                idUser: options.idUser,
            },
        ...
        let { success, message, historys } = results.data
        if (success) {
            await commit(types.UPDATE_INTEREST_LIST, historys.reverse())
        }
    },
    async getAdminEwallet({ commit, state }) {
        let results = await axios({
            method: 'POST',
            url: `${config.host}/user/getBitcoinAddress`,
            data: {
                // login_token: VueCoookie.get('login_token'),
                // idUser: 1,
            },
        })
        let { success, message, addressAdmin } = results.data
        if (success) {
            await commit(types.UPDATE_ADMIN_EWALLET, addressAdmin)
        }
    },
    async getListReference({ commit, state }, options) {
        try {
            let results = await axios({
                method: 'POST',
                url: `${config.host}/user/getRefer`,
                data: {
                    login_token: VueCoookie.get('login_token'),
                    idUser: options.idUser,
                },
            })
            let { success, refers } = results.data
            console.log(refers)
            if (success) {
                await commit(types.UPDATE_USER_LIST_REFERENCE, refers)
            }
        } catch (err) {
            console.log(err)
        }
