    ...
    investment: async (req, res, next) => {
        ...
        Bagbitcoin.update({ id_user: user.id }, {
            ...
            if (err)
                ...
                if(bag.investment <= 0)
                {
                    Bagbitcoin.update({ id_user: user.id },{period : 300}).exec((err, updatedPeriod) => {
                        if(err)
                            console.log('Failed to update period for bagbitcoin. '+err);
                        if(updatedPeriod.length <= 0)
                            console.log('Not found bagbitcoin to update period.');
                        else
                            console.log('Updated period for bagbigcoin.');
                    });
                }
                Transaction.create({
                    id_user: user.id,
                    ...
                    note: 'Type : Investment. Noti:' + (err || 'Success'),
                }).exec((err, his) => {
                    if (err)
                        console.log('Failed history investment. ' + err);
                    else
                        console.log('Created history investment');
                });
                ...
                if (userRef) {
                    ...
                    Bagbitcoin.update({ id_user: userRef.id }, {
                        ...
                        Historyinterestreceived.create({
                            id_user: userRef.id, interset_received: 0.05, pv_received: pvReceived,
                            name: user.username,
